...
%
% DESCRIPTION
% Plot the ROC curve E according to the 'traditional' way: on the x
...
% or the hit-rate/false alarm rate.
%
% SEE ALSO
% dd_error, dd_roc, dd_prc, dd_auc, getrocw
...
case 'prc'  % precision recall
   ...
   axis([0 1 0 1]);
case 'hit'  % hit-rate vs. false alarm
	h = plot(e(:,1), e(:,2),mrk);
	set(h,'tag','curve');
	xlabel('Hit rate','fontsize',fs);
	ylabel('False alarm rate','fontsize',fs);
   axis([0 1 0 1]);
case 'precisionatk'   % precision versus rank
   h = plot(e(:,1), e(:,2), mrk);
...
if ~isempty(op)
	...
	if ~isempty(h_old) % we have to remove it?
		%delete(h_old); % let's not remove, but rename:
      set(h_old,'tag','OPold');
	end
	set(h_op,'tag','OP');
