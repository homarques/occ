...
//			System.out.println(WARNING_MESSAGE);
	...
	/**
	 * Produces the outlier score for a single point in the data set, which is the point being analyzed
	 ...
	 * @throws IOException If any errors occur opening or writing to the output file
	 */
	public static OutlierScore calculateOutlierScore(ArrayList<Cluster> clusters, 
			double[] pointNoiseLevels, int[] pointLastClusters, double[] coreDistances, 
			int i, int pos) {
		
//		int numPoints = pointNoiseLevels.length;
		
		double epsilon_max = clusters.get(pointLastClusters[pos]).getPropagatedLowestChildDeathLevel();
		double epsilon = pointNoiseLevels[pos];
			
		double score = 0;
		if (epsilon_max == Double.MAX_VALUE)
			epsilon_max = clusters.get(pointLastClusters[pos]).getDeathLevel();
		if (epsilon != 0)
			score = 1-(epsilon_max/epsilon);
			
		OutlierScore Score = new OutlierScore(score, coreDistances[pos], i);
	...
	/** 
	 * 
	 * @param outlierScores A list of Outlier Scores already computed
	 * @param outlierScoresOutputFile The path to the outlier scores output file
	 * @param delimiter The delimiter for the output file
	 * @param infiniteStability true if there are any clusters with infinite stability, false otherwise
	 * @throws IOException If any errors occur opening or writing to the output file
	 */
	public static void writeOutlierScores(ArrayList<OutlierScore> outlierScores, String outlierScoresOutputFile,
			String delimiter, boolean infiniteStability) throws IOException {
		...
		for (OutlierScore outlierScore : outlierScores) {
			writer.write(outlierScore.getScore() + "\n");
		}
		writer.close();
