...
%
% INPUT
%   LABELS,LABELS1,LABELS2  Array of labels. Every row of LABELS is
%                           considered as a label of an object. It can be a 
%                           row vector of doubles or a character string.
%
% OUTPUT
...
function [out1,out2,out3] = renumlab(slab1,slab2)
	...
	elseif nargout > 1 || nargin == 1
		...
		if ischar(slab1), slab = char(slab1,slab2); else, slab = [slab1;slab2]; end
...
function slab = clean_lab(slab)
	...
	elseif isempty(slab)
		;
	elseif (size(slab,2) == 1) && (~ischar(slab))  	% Vector of numeric labels.
		J = isnan(slab);
		slab(J) = -inf;
	elseif (ischar(slab))     % Array of string labels.
		;
	else
...
% label lists) or -inf (in numeric label lists) and remove them.
	...
	elseif (ischar(lablist))    % Array of string labels.
