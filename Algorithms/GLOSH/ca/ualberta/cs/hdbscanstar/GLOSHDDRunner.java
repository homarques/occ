...
/**
 * Entry point for the GLOSHDD algorithm.
 * @author zjullion
 * @author swersky
 */
public class GLOSHDDRunner {
	
	private static final String TRAIN_FLAG = "train=";
	...
	public static void main(String[] args) {
		...
		System.out.println("Running GLOSHDD on training data " + parameters.trainFile + 
				", with test data " + parameters.testFile + " with minPts=" + parameters.minPoints + 
		...
		try {
			trainingSet = HDBSCANStar.readInDataSet(parameters.trainFile, ",");		
		}
		catch (IOException ioe) {
			System.err.println("Error reading training data set file.");
			System.exit(-1);
		}
		int numTrainingPoints = trainingSet.length;
		
		//Read in testing file:
		double[][] testSet = null;
		try {
			testSet = HDBSCANStar.readInDataSet(parameters.testFile, ",");		
		}
		catch (IOException ioe) {
			System.err.println("Error reading test data set file.");
			System.exit(-1);
		}
...
//		long startTime;
		...
		for (int i=0; i < numTestPoints; i++) {
			...
			double[][] ddSet = Arrays.copyOf(trainingSet, numTrainingPoints+1);
			ddSet[numTrainingPoints] = testSet[i];
			
			//Compute core distances:
//			startTime = System.currentTimeMillis();
			coreDistances = HDBSCANStar.calculateCoreDistances(ddSet, parameters.minPoints, parameters.distanceFunction);
//			System.out.println("Time to compute core distances (ms): " + (System.currentTimeMillis() - startTime));
	
			//Calculate minimum spanning tree:
//			startTime = System.currentTimeMillis();
			UndirectedGraph mst = HDBSCANStar.constructMST(ddSet, coreDistances, true, parameters.distanceFunction);
...
//			System.out.println("Time to calculate MST (ms): " + (System.currentTimeMillis() - startTime));
			...
			double[] pointNoiseLevels = new double[numTrainingPoints+1];
			int[] pointLastClusters = new int[numTrainingPoints+1];
...
//				System.out.println("Time to compute hierarchy and cluster tree (ms): " + (System.currentTimeMillis() - startTime));
			...
			//Propagate clusters:
			boolean infStab = HDBSCANStar.propagateTree(clusters);
			if (infStab)
...
//						",", numTrainingPoints+1, infiniteStability);
...
//			}
			...
			//Compute outlier score for current test point:
			outlierScores.add(HDBSCANStar.calculateOutlierScore(clusters, pointNoiseLevels, pointLastClusters, 
					coreDistances, i, numTrainingPoints+1));
//				System.out.println("Time to compute outlier scores (ms): " + (System.currentTimeMillis() - startTime));			
		...
		try {
			HDBSCANStar.writeOutlierScores(outlierScores, parameters.outlierScoreFile, ",", infiniteStability);
		}
		catch (IOException ioe) {
	...
	private static HDBSCANStarParameters checkInputParameters(String[] args) {
		...
		for (String argument : args) {
			...
			//Assign training input file:
			if (argument.startsWith(TRAIN_FLAG) && argument.length() > TRAIN_FLAG.length())
				parameters.trainFile = argument.substring(TRAIN_FLAG.length());
			
			//Assign testing input file:
			else if (argument.startsWith(TEST_FLAG) && argument.length() > TEST_FLAG.length())
				parameters.testFile = argument.substring(TEST_FLAG.length());
		...
		String inputName = parameters.testFile;
		if (parameters.testFile.contains("."))
			inputName = parameters.testFile.substring(0, parameters.testFile.lastIndexOf("."));
