...
function [p,I,J,d] = selproto(varargin)
...
% 	if (iscell(a))  % unfinished implementation
%     [p,I,J,d] = feval(mfilename,a,n,type,par,seed);
%     return
%   end
...
%% the real work
  ...
  switch lower(type)
    ...
    case {'f','fft','w','wft'}                             % FFT  
      fft = strncmpi(type,'f',1);
      if ~randsel
        [~,init] = min(distm(x,mean(x)));
      else
        init = [];
      end
      [p,I,J,d] = selprotf(x,n,[],type,init);
